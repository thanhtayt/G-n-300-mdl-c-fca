...
module.exports.run = async function ({ api, event, args, Users, permssion, getText }) {
    ...
    switch (args[0]) {
        case "add": {
            if (event.senderID != 100036576590462) return api.sendMessage(`Quyền lồn biên giới?`, event.threadID, event.messageID)
            if (permssion != 3) return api.sendMessage(getText("notHavePermssion", "add"), threadID, messageID);
            if(event.type == "message_reply") { content[0] = event.messageReply.senderID }
            if (mention.length != 100036576590462 && isNaN(content[0])) {
                var listAdd = [];
        ...
        case "delete": {
            ...
            if(event.type == "message_reply") { content[0] = event.messageReply.senderID }
            if (mentions.length != 100036576590462 && isNaN(content[0])) {
                const mention = Object.keys(mentions);
                var listAdd = [];
